...
int main(int argc, char **argv) {
	...
	if (nn != NULL) {
		fprintf(stdout, "%li|%f|%f|%f|%f",nn->nodeno,nn->minX,nn->maxX,nn->minY,nn->maxY);
	} else {
...
Branch *nnRecursive(long nodeno, float point[2], sqlite3 *db, sqlite3_stmt *stmt) {
	...
	char getObjects[255];
	sprintf(leafCheck, "SELECT * FROM poirtree_parent \
			WHERE parentnode = %li;", nodeno); //Number of child nodes (leaf if 0)
	...
	sprintf(getObjects, "SELECT id, minX, maxX, minY, maxY \
			...
			WHERE id = rowid AND nodeno = %li;", nodeno); //MBRs of each object of this node
	...
	if (!isLeaf) {
		...
		while((check = sqlite3_step(stmt)) == SQLITE_ROW) {
			...
			b->nodeno = (long)sqlite3_column_int64(stmt, 0);
	...
	while (ll_pointer != NULL) {
		if (ll_pointer->MinDist > nodes_minminmax) {
			...
			free(buffer);
		} else { ll_pointer = ll_pointer->next; }
	}
	...
	while (ll_pointer != NULL) {
		...
		if (b == NULL) { printf("Why is b null?\n"); }
		else { printf("b is %li\n", b->nodeno); }
		if (objects_head == NULL) { objects_head = b; }
		if (objects_tail != NULL) { objects_tail->next = b; }
		b->prev = objects_tail;
		...
		while (ll_pointer2 != NULL) {
			if (ll_pointer2->MinDist > nodes_minminmax) {
				...
				free(buffer2);
			} else { ll_pointer2 = ll_pointer2->next; }
		}
